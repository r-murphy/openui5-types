 ...
 */
function UI5(name: string): ((target: FunctionConstructor) => FunctionConstructor) {
    return function(target: FunctionConstructor): FunctionConstructor {
        var functionMembers: string[] = Object.getOwnPropertyNames(function() {}); // tslint:disable-line no-empty
        ...
        var thisClass: any = {};
        staticMembers  .forEach((m) => thisClass[m] = (target as any)[m]);
        instanceMethods.forEach((m) => thisClass[m] = (target.prototype as any)[m]);
