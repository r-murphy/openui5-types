...
export default class TreeBuilder {
    ...
    public static createFromSymbolsArray(config: Config, symbols: ui5.UI5Symbol[]): TreeNode[] {
    ...
    private static createNodeChildren(config: Config, symbols: ui5.UI5Symbol[], indentationLevel: number): TreeNode[] {
        let nodes: TreeNode[] = [];
        ...
        for (let namespace of namespaces) {
            if (!config.ignore.ignoreNamespaces.has(namespace)) {
                let parentSymbol = symbols.find((s) => s.name === namespace) || {
    ...
    private static createNode(config: Config, symbol: ui5.UI5Symbol, children: TreeNode[], indentationLevel: number): TreeNode {
        switch (symbol.kind) {
            ...
            default: throw new Error(`Unknown symbol kind: ${(symbol as any).kind}`);
        }
    }
}
